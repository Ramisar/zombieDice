...
public static void menu(){
	...
	while (counter != 1){
		...
		//Var `Char` instead of integer because I was trying to avoid crash the game when the user
...
public static void tutorial(){
	...
	while(counter != 1){
		...
		if (skip_tutorial == '2'){
			...
			System.out.println("      - Green: the easiest.  They have 1 Shotgun side, 2 Footprints sides and 3 Brains sides");
			System.out.println("      - Yellow: the medium tough. They have 2 Shotguns sides, 2 Footprints sides and 2 Brains sides");
...
public static void newgame(){
	...
	// Player index [0] in the ArrayList will correspond to Score [0] in the Array above
		...
		while(playerScore[turn] < 13){
			...
			playerOption = option.nextInt();
				...
				if (playerOption == 2){
					...
					// Statement created to keep the 'turn' level linked with the Arrays's index. Creating a looping.
					if(turn == totalPlayers){
						turn = 0;
					}
				
				// Breaking this looping so the program can move back to the previous looping and restart the process, resulting
			...
			if (playerOption == 1){
				...
				// The logic here is deduce the total of footprints from the total of dices per turn (3)
				// So I can play the correct number of dices per turn. Saving the footprints to be played separately
				...
				// Counter created because otherwise the lopping would create more dices if used the 'footprints' variable.
				int footCounter = footprints;
				
				//Looping used to check number of footprints and print the specific color 
				while (footCounter > 0){
					
					//Checks how many green footprints the user has and print a new random greendice.
					if (diceGreen > 0){
						diceGreen--;
						footprints--;
						footCounter--;
					...
					//Checks how many yellow footprints the user has and print a new random yellowdice.
			...
			/* 	After check how many dices the user has deducting the footprints;
				And after playing the specific number of footprints;
				The program will check the random number generate between 1-13 for  dicesRolling and apply one of the statements below.
			...
			*/
			...
			if (lives <= 0 ){
				...
				System.out.println("*                 Total Brains Saved: "+playerScore[turn] + "                *");
				System.out.println("********************************************************");
...
public static void playersName(){
		...
		//Clears players list before getting names because the user could be playing for the second time.
		players.clear();
...
public static void green_dice(){
	...
	//I have chosen to use Strings instead of numbers just because it is easier to read the code this way
